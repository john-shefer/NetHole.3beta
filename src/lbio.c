		  ...
		  const uint16_t ipl, const u_char *msg)
{
  ...
  if (ctl.mode & FL_RAND_MAC) {
    ip_addr_t offset;
    lbio_ip_offset(pkt->pkt_ip->ip_dst, &offset);
    eth_pack_hdr( new->pkt_eth,
                  pkt->pkt_eth->eth_src, /* orig src MAC becomes new dest MAC */
                  ctl.mac_array[offset],    /* rand mac becomes new src MAC */
                  ETH_TYPE_IP);
  }
  else {
    eth_pack_hdr( new->pkt_eth,
...
lbio_send_bogus_ARP (const ip_addr_t tpa, const struct pkt *pkt)
{
  ...
  if (ctl.mode & FL_RAND_MAC) {
    ip_addr_t offset = tpa - ctl.base;
    arp_pack_hdr_ethip(bogus->pkt_arp,
                       ARP_OP_REPLY,
                       ctl.mac_array[offset],     /* Bogus MAC is the "sender" */
