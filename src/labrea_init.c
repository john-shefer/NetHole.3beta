...
Usage (void)
{
  ...
  warnx(
	...
	"--rand-window (-W)      : Generate random window size\n" 
	"--rand-mac (-M) <key>   : Generate random MAC address (key: rand, vend=file_name)\n" 
	"--double-ack (-A)       : Double TCP-ACK\n" 
	"--tcp-opt (-C)          : Generate tcp options\n" 
...
void demo_init()
{
  /* Old version with signals -- doesn't work */
 /* pthread_t t0;
  ...
  // char msg[256];
  // sprintf(msg, "Empty string, for a test\n");
  if (pthread_create(&t0, NULL, draw_demo_window, NULL) == -1)
    warnx("Can't create thread for demonstration!");      */
  if (g_thread_new("demonstration", draw_demo_window, NULL) == NULL)
    warnx("Can't create thread for demonstration!");
}
...
labrea_init (int argc, char **argv)
{
  ...
  char *subopts, *value; // Считывают значение ключей для опций 

  /* parse the command line */
  while(TRUE){
    c = getopt_long(argc, argv, "n:m:i:j:I:E:qF:t:r:sM:AXWSCxhRHp:bPaflvoOVTdz?2:3Dey",
    ...
    switch(c) {
    case 'A': 
        ctl.mode |= FL_DOUBLE_ACK; 
        ctl.last_output_tcp = time(NULL); 
        break; 
    case 'M':
      ctl.mode |= FL_RAND_MAC;
      switch(getsubopt(&optarg, (char **)variations_opt_M, &value)) { 
        case RANDOM: 
            ctl.vendor_file = NULL; 
            break; 
        case VENDOR: 
            if(value == NULL) { 
              input_error = TRUE; 
              warnx("*** c ключом vend необходимо указать файл с вендорами (vend=filename)"); 
            } 
            ctl.vendor_file = value; 
            break; 
        default: 
            input_error = TRUE; 
            warnx("*** с опцией M необходимо указать один из ключей rand или vend (-M rand/vend)"); 
          break; 
      } 
      break;
    case 'W':
      ctl.mode |= FL_RAND_WINDOW;
      break;
    case 'C':
      ctl.mode |= FL_TCP_OPTS;
      break;
    case 'S':
      ctl.mode |= FL_RESTRICT_PORTS;
      break;
    case 'e':
      if (XOpenDisplay(NULL) != NULL){ /* Checking if X window system is available */
        ctl.mode |= FL_DEMOMODE;
      }
      else {
        warnx("Demonstration mode is unavailable under CLI");
        input_error = TRUE;
      }
      break;
    case 'X':
    ...
    case 'y': //Наборы открытых портов
      ctl.mode |= FL_OPEN_PORTS;
      break;
    case 't':
  ...
  if (ctl.mode & FL_RAND_MAC) {
    ...
    strncpy(bpf, "arp or ip", strlen("arp or ip"));
  }
  ...
  if (ctl.mode & FL_DEMOMODE) {
    util_print(NORMAL, "Demonstration mode is enabled");
    demo_init();
  }
...
//  char *file_name = "str.xml";
  //char *new_file_name = "new_str.xml";
 // Port_set prt[10];
 // int count = util_read_xml_structures(prt, file_name); /*Структура наборов открытых портов заполнена из xml файла*/

/*Проверка заполености структуры*/
//  int i=0;
 // int j=0;
//printf("*************\n");
/*for(i=0;i<count;i++)
{
printf("corteg %d => %s \n",prt[i].set_id, prt[i].set_description);
for(j=0;j<5;j++)
  {
    printf("port - %d  num - %d - %s \n",prt[i].port[j].port_id,prt[i].port[j].port_num,prt[i].port[j].port_description);
