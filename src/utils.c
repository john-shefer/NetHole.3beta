...
//считывает из файла октеты 
 
int 
util_read_vendor_list(void) 
{ 
  int  num_oktet = count_line_in_file(ctl.vendor_file); 

  FILE *file_with_vendor; 

  file_with_vendor = fopen(ctl.vendor_file, "r"); 

  if((ctl.oktet_array = calloc(num_oktet, sizeof(*ctl.oktet_array))) == NULL) { 
      warnx(  "*** calloc error: oktet_array - unable to allocate memory" ); 
      util_clean_exit(1); 
  } 

  int i = 0; 

  while(fscanf(file_with_vendor, "%2hhx%2hhx%2hhx", &ctl.oktet_array[i].data[0], &ctl.oktet_array[i].data[1], &ctl.oktet_array[i].data[2]) != EOF) 
      ++i; 

  fclose(file_with_vendor); 
  return num_oktet; 
} 
...
util_clean_exit(int err) 
{
  ...
  if (ctl.mac_array != NULL) {
    MYFREE(ctl.mac_array);
  }
...
/*
 * Check that random mac address is valid and not the same as any of macs of new kids
 *
 * Check that the Ethernet address (MAC) is not 00:00:00:00:00:00, is not
 * a multicast address, and is not FF:FF:FF:FF:FF:FF.
 *
 *
 * returns:   1 invalid mac
 *            0 valid mac
 *
 */
int
util_check_mac_valid (int offset)
{
  int res = 0;
  int ip;
  nk_t *nk;
  uint8_t *mac = &ctl.mac_array[offset].data;

  if ((0x01 & (*mac >> ((sizeof(*mac) * 8) - 8))) ||
      (((*(const uint32_t *)mac) | (*(const uint16_t *)(mac + 4))) == 0)) {
    res = 1;
    goto EXIT_LABEL;
  }
  ...
  for (ip = 0; ip < ctl.addresses + 2; ip++) {
    ...
    if ((nk = ctl.nk_array[ip]) != NULL &&
        (addr_cmp(&(nk->nk_mac), &rand_bogus_mac) == 0)) {
        res = 1;
        goto EXIT_LABEL;
    }
  }
...
/* 
 * Generation vendor mac addresses 
 */ 
 
void util_generator_mac_addr() { 
  int num_oktet;
  if(!ctl.vendor_file == NULL)
    num_oktet = util_read_vendor_list(); 
  ...
  for (ip = 0; ip < ctl.addresses + 2; ip++) 
    while(1) { 
      ...
      if(!ctl.vendor_file == NULL) {
        int i = rand()%num_oktet; 
        
        ctl.mac_array[ip].data[0] = ctl.oktet_array[i].data[0]; 
        ctl.mac_array[ip].data[1] = ctl.oktet_array[i].data[1]; 
        ctl.mac_array[ip].data[2] = ctl.oktet_array[i].data[2]; 
      }
 ...
 * Get timestamp value
 *
 * timestamp ≈ uptime * kernel frequency
 *
...
get_timestamp()
{
    ...
    if (error != 0) {
        util_print(NORMAL, "Sysinfo error...");
        return 1;
    }
    return (uint32_t)s_info.uptime * CONFIG_HZ;
}
